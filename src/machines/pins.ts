...
export const pinManagementMachine = createMachine<BaseContext, BaseEvent>({
...
function savePin(context: BaseContext, event: any) {
  context.data = {
    ...(context.data || {}),
    pins: {
      ...(context.data?.pins || {}),
...
function saveValidatedWard(context: BaseContext, event: any) {
  context.data = {
    ...(context.data || {}),
    pins: {
      ...(context.data.pins || {}),
...
async function authorizePinChange(context: BaseContext, event: any) {
...
async function validateWardToReset(context: BaseContext, event: any) {
  const { user: { account: { phone_number } } } = context;
  const { input } = event
  const ward = await validateTargetUser(context, input);
  const { account: wardAccount } = ward;
  const guardians = wardAccount.guardians || []
